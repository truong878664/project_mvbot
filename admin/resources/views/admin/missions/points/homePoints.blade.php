...
@section('title', 'Missions - Create Points')

@section('content')
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-10">
        <h1 class="h1 mb-0 text-gray-800">Missions - Create Points - List Robot</h1>
    </div>
        ...
        @if ($listsRobot->count() > 0)
            @foreach ($listsRobot as $robot)
                <div class="col-6 col-md-6 col-lg-4">
                    <a href="{{route('admin.missions.points.createPoints', $robot)}}" class="text-center" style="text-decoration: none;">
                            ...
                            <div class="card-header px-4 pt-4 text-center">
                                <div class="card-body pt-1">
                                    @if ($robot->mode == 'navigation')
                                        <i class="align-middle me-2 fas fa-fw fa-robot" style="font-size: 50px; color: #3b7ddd;"></i>
                                    @else 
                                        <i class="align-middle me-2 fas fa-fw fa-robot" style="font-size: 50px; color: #1cbb8c;"></i>
                                    @endif
                                </div>
                            </div>
                            @if ($robot->mode == 'navigation')
                                <h3 class="card-title mb-4 text-center" style="font-size: 20px; color: #3b7ddd;">{{$robot->serial}}</h3>
                            @else 
                                <h3 class="card-title mb-4 text-center" style="font-size: 20px; color: #1cbb8c;">{{$robot->serial}}</h3>
                            @endif
                        </div>
                    </a>
                </div>
