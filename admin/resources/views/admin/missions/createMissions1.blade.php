...
@section('content')
    ...
    <div class="row" style="margin-top: 48px;">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title fs-1">Group Missions</h5>
                <h6 class="card-subtitle text-muted fs-5">List Missions</h6>
            </div>
    ...
    @section('script')
        ...
        <script>
            ...
            function objectToString (obj) {
                ...
                let name_action = document.getElementById('name_action').value = name;
                let name = document.getAttribute('name');
                public function getName()
                {
                    var name = [];
                    if name_action = document.getElementById('name_action').value = name :
                    {
                        name_action = getName('name');
                    }
                    else{
                        name_action = getName('name_action');
                    }
                }
                public function get_arr_names ()
                {
                    var arr = [];
                    if arr_name = document.getElementById('name_arr').value = name :
                    {
                        arr.push(name);
                        arr_name = get_arr_names('name_arr');
                    }
                    var obj = { name_arr_action: ''};
                    obj.foo = 'Name action';
                    for(key in obj)
                    ...
                    console.log(varName.Array[]);
                    console.log(var.slice());
                    //const newStr = str.replace(regexp|substr, newSubstr|function)
                    console.log(newStr);

                    let mode = getModenameFromObject(nameObject);
                    function getModeName()
                    {
                        for (var name in mode)
                        {
                            if (mode[name] == nameObject)
                            return (name);
                        }
                    }
                    return ("");
                    let time_out = getTimeoutFromObject(nameObject);
                    function getTimeout()
                    {
                        for (var number in timeout)
                        {
                            if(timeout[number] == nameObject)
                            return (number);
                            console.log(number);
                        }
                    }
                    let data = getdataFromObject(nameObject);
                    function getData()
                    {
                        for (var data in datas)
                        {
                            if(datas[data] == nameObject)
                            return (data);
                            console.log(data);
                        }
                    }
                    function add ($Request $req)
                    {
                        ...
                        document.getAttribute('action').value = $action;
                        return $action;
                        @foreach ($req as $action)
                        {
                            $action = document.getModeName('action');
                            {{$action}}
                        @endforeach
                        }
                        return('page.missions-actions'), compact('actions', 'action', 'modename');
                        console.log(action);
                    }
                    function update ( $Request $req)
                    {
                        var action = $req ->getData();
                        document.querySelector('name').value = $action;
                        let obj = [];
                        const myObj = obj.split(" ");
                        document.getElementById("test").innerHTML = myObj[1];
                        var action = document.getElementById("test").innerHTML ;
                        var action = document.getElementById("test").innerHTML ;
                        ...
                        var time_out = [];
                        for ( var $time_out in myObj) {
                            time_out.push($time_out);
                            values.push(myObj[$time_out]);
                            return [time_out, values];
                            console.log(time_out);
                        }
                        var mode = [];
                        for ( var $mode in myObj) {
                            mode.push($mode);
                            values.push(myObj[$mode]);
                            return [mode, values];
                            console.log(mode);
                        }
                        var data = [];
                        for (var $data in myObj) {
                            data.push($data);
                            values.push(myObj[$data]);
                            return [data, values];
                            console.log(data);   
                        }
                        console.log(values);
            ...
            var arrObj = arr.map(funcion(key) {
                return {[key] : obj[key]};
            });
            ...
            function savename() {
                ...
                var index = s.indexOf("action");
                split(s, " ");
                console(s);   
            }
            const myArray = [];
            console.log(myArray.toString()); // Convert object{} to strings[]
            var = myArray + 's';
            if myArray = onclick('button_send'){
                ...
                document.write(getModeName(name));
